...
app = modal.App("stenosis-training")
...
image = (
    ...
    .pip_install(
        ...
        "kagglehub", "numpy", "matplotlib", "tqdm" # Added matplotlib and tqdm
    )
    .add_local_python_source("base_CNN", "train_base_csv_attention") 
)
...
CONFIG = {
    ...
    "save_path": "/root/models", # This will be our persistent volume mount point
}
...
@app.function(
    ...
    gpu="T4",
    volumes={
        "/data": data_volume,    # Where your CSVs live
        "/root/models": model_volume  # Where your .pth files will be saved
    },
    timeout=3600,
    secrets=[modal.Secret.from_name("kaggle-secret")]
)
def train():
    # Standard PyTorch imports inside the container
    import torch
    import kagglehub
    ...
    # Custom imports from your synced project files
    from base_CNN import StenosisCNN
    ...
    # Only download if the images aren't already in our volume
    if not os.path.exists(train_img_dir):
        ...
        # Move it to our persistent volume so it stays there forever
        import shutil
        os.makedirs(persistent_data_path, exist_ok=True)
        # Copy contents from temp to /data (dirs_exist_ok merges contents correctly)
    ...
    train_dataset = StenosisDataset(train_csv, train_img_dir)
    val_dataset = StenosisDataset(val_csv, val_img_dir)
    
    # Define Transforms (Augmentation for Train, Normalization for all)
    ...
    model = StenosisCNN().to(device)
    
    criterion = nn.CrossEntropyLoss()
    # Calculate Class Weights to fix the "66% stagnation"
    # If class 0 is rare, we give it a higher weight so the model cares more about it
    ...
    # Reduce LR if accuracy stagnates
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1, patience=3)

    print(f"--- 4. Starting Training for {CONFIG['epochs']} epochs ---")
    best_val_acc = 0.0
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if val_acc > best_val_acc:
            ...
            model_volume.commit() # Important: Write the file to permanent storage
        
        # Periodic Checkpoints
        if (epoch + 1) % CONFIG["checkpoint_interval"] == 0:
            ckpt_path = os.path.join(CONFIG["save_path"], f"checkpoint_epoch_{epoch+1}.pth")
            torch.save({
                'epoch': epoch,
    ...
    # Save to your persistent Volume mount point
    plot_path = os.path.join(CONFIG["save_path"], "training_curves.png")
    plt.savefig(plot_path)
    print(f"Plots saved to {plot_path}")
    
    # Final sync to the cloud
    model_volume.commit()
